---
layout: about
permalink: /about/
title: About
heading: ABOUT ME
number: "02"
description: >-
  Abhay Tharakan — mechanical engineering at Purdue, with a background in CAD,
  fabrication, embedded electronics, and competitive swimming.

photo: /assets/images/abhay.jpg
photo_alt: Abhay Tharakan, standing outdoors in a blue patterned shirt

skills:
  - SolidWorks
  - Fusion 360
  - Siemens NX
  - 3D Printing
  - Arduino
  - Circuit Prototyping
  - Python
  - MATLAB
  - C
  - HTML

experience:
  - role: Undergraduate Research Assistant
    org: Purdue University
    period: JAN 2026 — MAY 2026
    points:
      - Applied industrial engineering methodologies to transform unstructured
        surgical transcripts into structured datasets for research use.
      - Analyzed qualitative datasets to identify patterns in efficiency, clarity,
        and operational reliability.

  - role: Assistant Swim Coach
    org: Age-group swimming
    period: MAY 2025 — JUL 2026
    points:
      - Coached 110+ swimmers aged 6–18 across two seasons, developing stroke
        technique and race performance.
      - Coordinated meet operations — event lineups and communication across
        coaches, athletes, and parents.

  - role: Athletics Marketing Intern
    org: Apex High School
    period: AUG 2024 — JUN 2025
    points:
      - Ran social media for the athletics program across three platforms,
        producing content that grossed 40,000+ views.

education:
  - school: Purdue University
    location: West Lafayette, Indiana
    period: AUG 2025 — PRESENT
    detail: GPA 3.98 · Dean's List with Semester Honors

  - school: Apex High School
    location: Apex, North Carolina
    period: AUG 2021 — JUN 2025
    detail: GPA 4.22 · Magna Cum Laude · AP Scholar with Distinction
---


Outside the shop, I swam competitively through high school — a state title in the
100 backstroke and four years as a Scholastic All American — and I now coach
age-group swimmers. A lot of what I like about coaching is the same thing I like
about engineering: breaking something complicated into parts you can actually
work on.
